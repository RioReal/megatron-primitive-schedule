...
"""Single-worker SlackPipe schedule prototype."""
...
def forward_backward_slackpipe(
    *,
    ...
    """Run a validated SlackPipe plan on a single local worker.

    This milestone intentionally supports only PP=TP=CP=DP=1 and local
    execution. Stage tensors are keyed by (microbatch_id, stage_id) so the
    JSON plan can choose any dependency-valid operation order.
    """

    del seq_length, micro_batch_size, decoder_seq_length, p2p_communicator, force_all_reduce
    ...
    if len(model_chunks) != plan.num_stages:
        raise ValueError(
            "SlackPipe prototype requires one local model chunk per plan stage "
            f"({len(model_chunks)} != {plan.num_stages})"
        )
    ...
    with no_sync_func():
        for op in plan.local_operations:
            ...
            if op.kind == "F":
                ...
                output_tensor, num_tokens = forward_step(
                    forward_step_func,
                    data_iterators[op.stage],
                    model_chunks[op.stage],
                    num_microbatches,
                    input_tensor,
                    forward_data_store,
                    config,
                    ...
                    vp_stage=op.stage if plan.num_stages > 1 else None,
                    is_last_stage=op.stage == plan.num_stages - 1,
                )
                output_tensors[key] = output_tensor
                if op.stage != plan.num_stages - 1:
                    forwarded_output_tensors[key] = _detach_pipeline_tensor(output_tensor)
                ...
                if op.stage == plan.num_stages - 1:
                    ...
                    output_tensor_grad = input_tensor_grads[next_key]
...
def _validate_supported_parallelism() -> None:
    ...
    if pp_size != 1:
        raise ValueError(
            f"SlackPipe prototype requires pipeline model parallel size 1, got {pp_size}"
        )
    if tp_size != 1:
        raise ValueError(
